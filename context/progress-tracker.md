...
- **Carry-over blocker RESOLVED:** the Clerk v7 `afterSignOutUrl` error in `components/app-navbar.tsx` (×2) is fixed — `afterSignOutUrl="/sign-in"` moved to `<ClerkProvider>` in `app/layout.tsx`, removed from both `UserButton`s. Full `next build` now passes (exit 0) and Vercel deploys.
...
- **SEO/AEO/GEO hardening + branding:** company peacock logo is now the brand mark everywhere (site/app/auth/pending headers) and the favicon via file-based `app/icon.png` / `apple-icon.png` (replaced the default Next.js favicon). Closed the audit gaps: (1) `siteConfig.url` is now env-driven via `NEXT_PUBLIC_SITE_URL` (falls back to the prod domain) so every canonical/OG/sitemap/JSON-LD URL matches the deploy host — **must be set in Vercel env if the live domain isn't kumarannaturalproducts.com**; (2) explicit `alternates.canonical` on home/products/about/blog/blog-post/faq/contact; (3) `/price` placeholder set to `robots: index:false`; (4) generated 1200×630 OG card at `app/opengraph-image.tsx` (next/og, statically prerendered, embeds the logo) replacing the cropped square; (5) `LocalBusiness` JSON-LD on home (geo/areaServed/priceRange/contactPoint) alongside the existing Organization data; (6) `BreadcrumbList` on products + blog posts; (7) Product schema enriched with `image`+`sku`+offer url; (8) Article schema gained `dateModified` + structured publisher logo; (9) Twitter `summary_large_image` card + `og:locale` in root metadata. **Not done (larger refactor, deferred):** true `/[lang]` localized routing + `hreflang` — public trilingual content is still client-side only, so si/ta aren't separately indexable. `next build` passes (exit 0).

- **Ultra SEO/SEM/AEO/GEO pass (all pages):** Centralised every page's SEO into `lib/seo.ts` — a server-only module with schema.org node builders (`organizationNode`, `websiteNode` w/ SearchAction sitelinks box, `localBusinessNode` w/ geo, `webPageNode`, `breadcrumbNode`), a `graph()` `@graph` wrapper (shared `@id` refs), and a `pageMeta()` helper that emits canonical + Open Graph (with `si_LK`/`ta_LK` alternateLocale) + Twitter `summary_large_image` + `googleBot` max-preview directives. **Global identity graph** (Organization + WebSite) now renders once in `app/layout.tsx`; root metadata enriched (keywords, authors/creator/publisher, applicationName, category, referrer, formatDetection, manifest, viewport themeColor). Added **`app/manifest.ts`** (PWA web manifest → `/manifest.webmanifest`). Per-page graphs: Home (LocalBusiness + WebPage), About (AboutPage + Organization + breadcrumb), Contact (ContactPage + ContactPoint + breadcrumb), Products (CollectionPage + enriched ItemList/Product offers w/ NewCondition/seller/areaServed + breadcrumb), Blog list (Blog + BlogPosting list + breadcrumb), Blog post (Article w/ wordCount/keywords/inLanguage/isPartOf + breadcrumb), FAQ (FAQPage + WebPage + breadcrumb). `app/robots.ts` now has explicit allow rules for answer-engine crawlers (GPTBot, ClaudeBot, PerplexityBot, Google-Extended) + disallows `/price`; `app/sitemap.ts` gained `lastModified` + image entries. `/price` placeholder routed through `pageMeta({ noindex })`. **Still deferred:** true `/[lang]` localized routing + per-URL `hreflang` (trilingual content is still client-side off one URL, so OG `alternateLocale` is used rather than separate indexable si/ta URLs). `tsc --noEmit` clean.
